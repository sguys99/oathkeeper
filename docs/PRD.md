...
### 6.3 페이지 3: Deal 상세 분석 결과 페이지 (`/deals/:id`)
...
- 프롬프트 텍스트는 코드베이스 내 `backend/prompts/` 디렉터리에 `.txt` 파일로 관리
