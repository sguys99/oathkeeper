...
- [ ] `backend/pyproject.toml` — `langchain-ollama>=0.3.0` 의존성 추가
- [ ] `backend/app/utils/settings.py` — `llm_provider` Literal에 `"ollama"` 추가
- [ ] `backend/app/utils/settings.py` — Ollama 설정 필드 추가
  - `ollama_base_url: str = "http://localhost:11434"`
  - `ollama_model: str = "qwen2.5:14b"`
- [ ] `backend/app/agent/llm.py` — `ChatOllama` import 및 라우팅 분기 추가
  - `num_predict=max_tokens` (Ollama는 `max_tokens` 대신 `num_predict` 사용)
- [ ] `.env.example` — Ollama 관련 환경변수 항목 추가
- [ ] 동작 검증
  - [ ] `ollama pull qwen2.5:14b` 후 서버 기동
...
| 모델 | 파라미터 | 컨텍스트 | 비고 |
|------|----------|----------|------|
| `qwen2.5:14b` | 14B | 32K | 권장 (품질 우수) |
| `qwen2.5:7b` | 7B | 32K | 경량 옵션 |
...
- [ ] `backend/app/utils/settings.py` — `prompt_profile: Literal["full", "compact"] = "full"` 추가
- [ ] `backend/app/agent/prompt_loader.py` — `load_prompt()` 함수에 profile 파라미터 지원
- [ ] `configs/prompts/final_verdict.yaml` — compact 섹션 추가 (목표: 50% 토큰 절약)
- [ ] `configs/prompts/resource_estimation.yaml` — compact 섹션 추가
- [ ] `configs/prompts/scoring.yaml` — compact 섹션 추가
- [ ] `configs/prompts/risk_analysis.yaml` — compact 섹션 추가
- [ ] `configs/prompts/deal_structuring.yaml` — compact 섹션 추가
- [ ] 동작 검증
...
- [ ] `backend/app/utils/settings.py` — 임베딩 설정 필드 추가
  - `embedding_provider: Literal["openai", "ollama"] = "openai"`
  - `ollama_embedding_model: str = "nomic-embed-text"`
  - `embedding_dimensions: int = 1536`
- [ ] `backend/app/agent/embeddings.py` — `OllamaEmbeddings` 라우팅 추가
- [ ] `backend/app/db/vector_store.py` — 차원 설정값 참조로 변경 (하드코딩 제거)
- [ ] `.env.example` — 임베딩 관련 환경변수 항목 추가
- [ ] 동작 검증
...
- [ ] 동작 검증
  - [ ] `NODE_MODEL_OVERRIDES='{"final_verdict": "qwen2.5:14b", "deal_structuring": "qwen2.5:7b"}'` 설정 후 분석 실행
...
| Phase | 상태 | 예상 소요 |
|-------|------|-----------|
| Phase 1: Ollama LLM 기본 연계 | ⬜ 미시작 | 1~2일 |
| Phase 2: SLM용 프롬프트 최적화 | ⬜ 미시작 | 1~2일 |
